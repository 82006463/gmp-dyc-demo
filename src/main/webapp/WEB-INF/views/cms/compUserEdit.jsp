...
<head>
	...
	<script type="text/javascript">
		function changeComp(curTag) {
			var _option = '';
			$('#compId').find('option:gt(0)').remove();
			if($(curTag).val() == 1) {
				<c:forEach items="${measureComps}" var="item">
					_option += '<option value="${item.id}">${item.name}</option>';
				</c:forEach>
			} else {
...
<body>
	<form id="inputForm" action="${ctx}/custom/cms/compUser/update" method="post">
		...
		<table class="table_all" align="center" border="0" cellpadding="0" cellspacing="0" style="margin-top: 0px">
			...
			<tr>
				<td class="td_table_1">公司类型<b class="requiredWarn">*</b>：</td>
				<td class="td_table_2">
					<input type="radio" id="compType1" name="compType" value="1" class="validate[required]" ${entity.compType==1 ? 'checked="checked"':''} onclick="changeComp(this);"/>&nbsp;计量公司
					<input type="radio" id="compType2" name="compType" value="2" class="validate[required]" ${entity.compType==2 ? 'checked="checked"':''} onclick="changeComp(this);"/>&nbsp;药企
				</td>
				<td class="td_table_1">公司<b class="requiredWarn">*</b>：</td>
				<td class="td_table_2">
					<select id="compId" name="compId" class="input_select validate[required]">
						<option value="">-请选择-</option>
						<c:if test="${entity.compType == 1}">
							<c:forEach items="${measureComps}" var="item">
								<option value="${item.id}" <c:if test="${entity.measureCompId==item.id}">selected="selected"</c:if>>${item.name}</option>
							</c:forEach>
						</c:if>
						<c:if test="${entity.compType == 2}">
							<c:forEach items="${drugComps}" var="item">
			...
			<tr>
				...
				<td class="td_table_2">
					<input type="text" id="email" name="email" value="${entity.email}" class="input_240 validate[required,custom[email]]" />
				</td>
			</tr>
		</table>
