...
<html>
	...
	<body>
	<form id="mainForm" action="${ctx}/custom/cms/measureComp" method="get">
				...
				<td align="left">
					<c:if test="${empty lookup}">
						<shiro:hasPermission name="cms_measureComp_edit">
							<input type='button' onclick="addNew('${ctx}/custom/cms/measureComp/create')" class='button_70px' value='新建'/>
						</shiro:hasPermission>
					</c:if>
					<c:if test="${!empty lookup}">
					...
					<td class="td_list_2" align=left>
						<c:if test="${empty lookup}">
							<c:if test="${item.status > 0}">
								<shiro:hasPermission name="cms_measureComp_delete">
									<a href="${ctx}/custom/cms/measureComp/delete/${item.id}" class="btnDel" title="删除" onclick="return confirmDel();">删除</a>
								</shiro:hasPermission>
								<shiro:hasPermission name="cms_measureComp_edit">
									<a href="${ctx}/custom/cms/measureComp/update/${item.id}" class="btnEdit" title="编辑">编辑</a>
								</shiro:hasPermission>
								<c:if test="${item.status == 1}">
									<shiro:hasPermission name="cms_measureComp_review">
										<a href="${ctx}/custom/cms/measureComp/update/${item.id}?op=2" class="btnEdit" title="复核">复核</a>
									</shiro:hasPermission>
								</c:if>
							</c:if>
							<a href="${ctx}/custom/cms/measureComp/view/${item.id}" class="btnView" title="查看">查看</a>
						</c:if>
						<c:if test="${!empty lookup}">
