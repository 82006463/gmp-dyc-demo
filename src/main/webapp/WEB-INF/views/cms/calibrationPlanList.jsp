...
<html>
	<head>
		<title>月度${fn:contains(type,'Ext') ? '外校':fn:contains(type,'In') ? '内校':fn:contains(type,'Tmp') ? '临校':''}${status==3 ? '任务':'计划'}</title>
	...
	<form id="mainForm" action="${ctx}/custom/cms/${type}/${status}" method="get">
		...
		<table width="100%" border="0" align="center" cellpadding="0" class="table_all_border" cellspacing="0" style="margin-bottom: 0px;border-bottom: 0px">
			<tr>
				<td class="td_table_top" align="center">月度${fn:contains(type,'Ext') ? '外校':fn:contains(type,'In') ? '内校':fn:contains(type,'Tmp') ? '临校':''}${status==3 ? '任务':'计划'}</td>
			</tr>
		</table>
