...
<html>
	...
	<body>
	<form id="mainForm" action="${ctx}/config/codeRule/list?lookup=${lookup }" method="get">
				...
				<td align="left">
					<c:if test="${empty lookup}">
						<%--<shiro:hasPermission name="config_codeRule_edit">--%>
							<input type='button' onclick="addNew('${ctx}/config/codeRule/create')" class='button_70px' value='新建'/>
						<%--</shiro:hasPermission>--%>
					</c:if>
					<c:if test="${!empty lookup}">
			...
			<c:forEach items="${page.result}" var="item">
				<tr>
					...
					<td class="td_list_2" align=left>
						<c:if test="${empty lookup}">
							<%--<shiro:hasPermission name="config_codeRule_edit">--%>
								<a href="${ctx}/config/codeRule/delete/${item.id}" class="btnDel" title="删除" onclick="return confirmDel();">删除</a>
							<%--</shiro:hasPermission>
							<shiro:hasPermission name="config_codeRule_edit">--%>
								<a href="${ctx}/config/codeRule/update/${item.id}" class="btnEdit" title="编辑">编辑</a>
							<%--</shiro:hasPermission>
							<shiro:hasPermission name="config_codeRule_view">--%>
								<a href="${ctx}/config/codeRule/view/${item.id}" class="btnView" title="查看">查看</a>
							<%--</shiro:hasPermission>--%>
						</c:if>
						<c:if test="${!empty lookup}">
