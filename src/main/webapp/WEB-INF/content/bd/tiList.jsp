...
<html>
	...
	<body>
	<form id="mainForm" action="${ctx}/bd/ti/list?lookup=${lookup }" method="get">
			...
			<tr>
				...
				<c:choose>
					<c:when test="${empty lookup}">
						<input type='button' onclick="addNew('${ctx}/bd/ti/create')" class='button_70px' value='新建'/>
					</c:when>
					<c:otherwise>
				...
				<tr>
					...
					<td class="td_list_2" align=left>
						<c:if test="${empty lookup}">
							...
							<a href="${ctx}/bd/ti/update/${item.id}" class="btnEdit" title="编辑">编辑</a>
							<a href="${ctx}/bd/ti/view/${item.id}" class="btnView" title="查看">查看</a>
						</c:if>
						<c:if test="${!empty lookup}">
