...
<body>
	<form id="mainForm" action="${ctx}/bd/trt/list?lookup=${lookup }" method="get">
			...
			<tr>
				...
				<c:choose>
					<c:when test="${empty lookup}">
						<input type='button' onclick="addNew('${ctx}/bd/trt/create')" class='button_70px' value='新建'/>
					</c:when>
					<c:otherwise>
				...
				<tr>
					...
					<td class="td_list_2" align=left>
						<c:if test="${empty lookup}">
							<a href="${ctx}/bd/trt/delete/${item.id}" class="btnDel" title="删除" onclick="return confirmDel();">删除</a>
							<a href="${ctx}/bd/trt/update/${item.id}" class="btnEdit" title="编辑">编辑</a>
							<a href="${ctx}/bd/trt/view/${item.id}" class="btnView" title="查看">查看</a>
						</c:if>
						<c:if test="${!empty lookup}">
