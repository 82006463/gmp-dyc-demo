...
public class CalibrationTmpController {
    ...
    public ModelAndView list(Page<CalibrationTmp> page, @PathVariable Integer status, Integer taskStatus, HttpServletRequest request) {
    ...
    public Object sendTask(HttpServletRequest req) {
        ...
        resultMap = calibrationTmpService.sendTask(user, null, approver);
        return resultMap;
    }
    ...
    public void exportFile(@PathVariable Integer status, HttpServletRequest req, HttpServletResponse resp) {
        ...
        try (OutputStream out = resp.getOutputStream()) {
            String fileName = URLEncoder.encode("月度临校" + (status.intValue() == 3 ? "任务-" : "计划-") + DateFormatUtils.format(new Date(), "yyyyMMddHHmmss") + ".xls", "UTF-8");
