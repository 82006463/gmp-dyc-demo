...
public class CalibrationExtController {
    ...
    public Object sendTask(HttpServletRequest req) {
        ...
        resultMap = calibrationExtService.sendTask(user, null, approver);
        return resultMap;
    }
    ...
    public void exportFile(@PathVariable Integer status, HttpServletRequest req, HttpServletResponse resp) {
        ...
        try (OutputStream out = resp.getOutputStream()) {
            String fileName = URLEncoder.encode("月度外校" + (status.intValue() == 3 ? "任务-" : "计划-") + DateFormatUtils.format(new Date(), "yyyyMMddHHmmss") + ".xls", "UTF-8");
            ...
            if (pageExt.getResult() != null && !pageExt.getResult().isEmpty()) {
                ...
                for (CalibrationExt tmp : pageExt.getResult()) {
