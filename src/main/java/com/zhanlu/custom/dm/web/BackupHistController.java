...
public class BackupHistController {
    ...
    public ModelAndView create() {
        ModelAndView mv = new ModelAndView("dm/backupHistEdit");
        BackupHist entity = new BackupHist();
        entity.setStatus(1);
    ...
    public ModelAndView update(BackupHist entity, HttpServletRequest req) {
        ...
        backupHistService.saveOrUpdate(entity);
        ModelAndView mv = new ModelAndView("redirect:/custom/dm/backupHist");
        return mv;
    }
    ...
    public Object check(Equipment entity, HttpServletRequest req) {
        ...
        if (entity.getId() == null || entity.getId().longValue() < 1) {
            ...
            BackupHist tmp = backupHistService.findById(entity.getId());
            if (!resultMap.containsKey("msg") && StringUtils.isNotBlank(entity.getCode())) {
                maps = jdbcTemplate.queryForList(sqlBuf.toString() + " AND code='" + entity.getCode() + "'");
